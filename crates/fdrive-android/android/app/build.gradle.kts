...
val cargoHostBuild = tasks.register<Exec>("cargoHostBuild") {
    ...
    commandLine("cargo", "build", "-p", "fdrive-android")
}
...
val generateUniffiBindings = tasks.register<Exec>("generateUniffiBindings") {
    ...
    workingDir = workspaceRoot
    commandLine(
        "cargo", "run", "-p", "fdrive-android", "--bin", "uniffi-bindgen", "--",
        "generate", "--library", "target/debug/libfdrive_android.so",
...
val cargoNdkBuild = tasks.register<Exec>("cargoNdkBuild") {
    ...
    commandLine(
        ...
        "build", "--release", "-p", "fdrive-android",
    )
}
