...
impl Adapter {
    async fn listing(&self, directory: &RelPath) -> Result<Vec<sdk::FileInfo>, FsError> {
        ...
        let listing = match cached {
            ...
            None => match self.engine.fs().ls(directory).await {
                Ok(fetched) => {
                    self.engine.view().note(directory, &fetched);
                    self.engine
                        .local()
                        .meta
                        .lock()
                        .unwrap()
                        .insert(directory.clone(), (Instant::now(), fetched.clone()));
                    fetched
                }
                ...
                Err(err) => {
                    let meta = self.engine.local().meta.lock().unwrap();
                    match meta.get(directory) {
                        Some((_, listing)) => {
                            log::debug!("ls {directory} unreachable, serving stale: {err}");
                            listing.clone()
                        }
                        None => {
                            drop(meta);
                            log::debug!("ls {directory} unreachable, serving the ledger: {err}");
                            self.engine.view().remembered(directory)
                        }
                    }
                }
            },
