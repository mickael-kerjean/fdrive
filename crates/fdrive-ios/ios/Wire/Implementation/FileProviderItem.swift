...
final class FileProviderItem: NSObject, NSFileProviderItem {
    ...
    init(path: String, parent: NSFileProviderItemIdentifier, entry: Entry) {
        ...
        var granted: NSFileProviderItemCapabilities = isDirectory ? [.allowsContentEnumerating] : []
        if entry.canRead { granted.insert(.allowsReading) }
        if entry.canWrite { granted.insert(isDirectory ? .allowsAddingSubItems : .allowsWriting) }
