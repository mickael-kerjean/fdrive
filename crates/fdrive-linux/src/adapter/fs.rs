...
impl<'a> Fs<'a> {
                ...
                Err(err) => {
                    ...
                    match meta.get(dir) {
                        ...
                        None => {
                            ...
                            self.0.engine.view().remembered(dir)
                        }
                    }
                }
            },
    ...
    pub fn attr(self, path: &RelPath) -> io::Result<Option<(bool, u64, SystemTime)>> {
        ...
        if let Some(md) = self.0.engine.view().pending_metadata(path) {
    ...
    pub fn opened(self, path: &RelPath, writable: bool) -> u64 {
        if writable {
            self.0.engine.fs().write_opened(path);
        }
    ...
    pub fn closed(self, fh: u64) {
        if let Some(handle) = self.0.handles.close(fh) {
            if handle.writable {
                self.0.engine.fs().write_closed(&handle.path);
            }
            self.0.engine.fs().released(&handle.path);
        }
    }

    pub fn read(self, fh: u64, path: &RelPath, offset: u64, size: u32) -> io::Result<Vec<u8>> {
        if let Some(download) = self.0.engine.cache().reader(path) {
    ...
    pub fn truncate(self, path: &RelPath, size: u64) -> io::Result<()> {
        if size > 0 {
            self.0.cache().hydrate(path)?;
        } else if self.0.engine.view().untracked(path) {
            self.0.engine.block_on(self.0.engine.view().overwriting(path));
        }
        let file_path = self.0.engine.local().backing(path);
        ensure_parent(&file_path)?;
        let file = fs::OpenOptions::new()
