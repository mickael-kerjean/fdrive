...
fn set_get_roundtrip() {
    ...
    assert_eq!(db.get(&path, "user.tag").as_deref(), Some(b"blue".as_slice()));
...
fn create_and_replace_flags() {
    ...
    let db = XattrDb::open(file.0.clone());
    let path = RelPath::new("f");
    assert_err(db.set(&path, "user.a", b"x", libc::XATTR_REPLACE), Errno::ENODATA);
    db.set(&path, "user.a", b"x", libc::XATTR_CREATE).unwrap();
    assert_err(db.set(&path, "user.a", b"y", libc::XATTR_CREATE), Errno::EEXIST);
...
fn remap_moves_the_subtree_and_drops_the_replaced_target() {
    ...
    db.set(&RelPath::new("z"), "user.k", b"replaced", 0).unwrap();
    db.remap(&RelPath::new("a"), &RelPath::new("z"));
    assert_eq!(db.get(&RelPath::new("a/x"), "user.k"), None);
    assert_eq!(db.get(&RelPath::new("z/x"), "user.k").as_deref(), Some(b"kept".as_slice()));
...
fn survives_a_reopen() {
    let file = TempFile::new();
    let path = RelPath::new("f");
    XattrDb::open(file.0.clone()).set(&path, "user.tag", b"blue", 0).unwrap();
    let db = XattrDb::open(file.0.clone());
    assert_eq!(db.get(&path, "user.tag").as_deref(), Some(b"blue".as_slice()));
}
